...
## 特徴
1. 鍵盤表示で演奏情報をグラフィカルに表示します
2. お気に入りを集めたプレイリストを作ることができます
3. ZIPファイル内のSMFを再生できます
4. ZIPファイル内のドキュメントを表示できます
5. ファイル名を変更せず、曲名を入力することができます

### ZIPファイルへの対応
バージョン3.0.0からZIPファイル内にあるSMFを再生できるようになりました。

以下の特徴・制限があります

1. SMF以外のファイルが含まれていても構いません
2. ZIPファイル内に複数のMIDIファイルがある場合、最初に見つかったファイルのみを再生します
3. ZIPファイル内にテキストファイルがある場合、コンテキストメニューの「ドキュメントを見る」が有効になり、別ウィンドウにドキュメントの内容を表示します

### アプリ内課金によるファイルの扱いの変更
バージョン4.0.0まではアプリケーションの制限区域内にファイルをコピーしていました。  
アプリ内課金をすることで、ユーザーが指定したフォルダ内のMIDIファイルを使用することができます。  
クラウドストレージを指定した場合、複数のMacで同じMIDIファイルを利用することができますし、入力した曲名も共有することも可能です。

## 使い方
本アプリケーションをダウンロードし、初めて使うときは利用したいMIDIファイルを本アプリケーションに追加します。

アプリ内課金をした場合は、MIDIファイルが入っているフォルダを選択します。
...
メニューバーの「ファイル＞MIDIファイルのフォルダを選ぶ...」を選びます。  
フォルダを選ぶためのパネルが開くので、ここからMIDIファイルが入っているフォルダを選んでください。

<p align="center">
<img width="300" height="246" alt="image" src="images/260.png" />
</p>
...
はできません。  
これらの作業はFinderで行ってください。  
現状はアンドゥをサポートしていないため、これらの作業はできないようにしています。
...
## 検索
バージョン3.0.0からファイルの検索機能が追加されました。

検索結果を再生したり、検索結果をプレイリストに追加することができます。
<p align="center">
<img width="300" height="221" alt="image" src="images/1250.png" />
</p>
